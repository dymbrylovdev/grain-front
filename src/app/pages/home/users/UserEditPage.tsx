...
//         accessTransporter() ? setValueTabs(2) : setValueTabs(3);
//       }
//     }
//   }, [match.url, editMode]);
...
//       if (newValue === 1) accessTransporter() ? history.push("/user/profile/options") : history.push("/user/profile/points");
...
//                       {...a11yProps(accessByRoles(me, ["ROLE_TRANSPORTER"]) ? 2 : 3)}
//                     />
//                   )}
...
const UserEditPage: React.FC<TPropsFromRedux & WrappedComponentProps & RouteComponentProps<{ id: string }>> = ({
  ...
  const handleTabsChange = (event: any, newValue: number) => {
    ...
    console.log("isAdminProfile", isAdminProfile());


    if (editMode === "profile") {
      if (newValue === 0) history.push("/user/profile");
      if (newValue === 1) accessTransporter()
        ? history.push("/user/profile/options")
        : isAdminProfile() ? history.push("/user/profile/points") : history.push("/user/profile/tariffs");
      if (newValue === 2) accessTransporter()
        ? history.push("/user/profile/tariffs")
        : isAdminProfile() ? history.push("/user/profile/crops") : history.push("/user/profile/points");
    ...
    <>
      ...
      <Paper className={classes.paperWithForm}>
        ...
        <div className={classes.form} style={valueTabs === 4 || valueTabs === 3 ? { maxWidth: 1000 } : undefined}>
          ...
          <AppBar position="static" color="default" className={classes.appBar}>
            {user || window.location.href.includes("profile") || window.location.href.includes("create") ? (
              <Tabs
                ...
                aria-label="tabs"
              >
                <Tab label={intl.formatMessage({ id: "USER.EDIT_FORM.PROFILE" })}
                  {...a11yProps(0)}
                />
                ...
                {accessByRoles(me, ["ROLE_ADMIN"]) && editMode === "create" && !user ? null : (me &&
                  ...
                  !isTransporterProfile() && <Tab className={classes.displayNone} label={intl.formatMessage({ id: "USER.EDIT_FORM.CROPS" })} {...a11yProps(2)} />
                )}
          ...
          {!accessByRoles(user, ["ROLE_TRANSPORTER"]) && !accessByRoles(me, ["ROLE_TRANSPORTER"]) && (
            <TabPanel value={valueTabs} index={editMode === "profile"
              ? isAdminProfile() ? 1 : 2
              : 4}>
              {editMode === "create" ? (
                <p>{intl.formatMessage({ id: "LOCATIONS.FORM.NO_USER" })}</p>
          ...
          {/* (editMode==='view' || editMode ==='profile')  */}

          {accessByRoles(me, ["ROLE_TRANSPORTER"]) || accessByRoles(user, ["ROLE_TRANSPORTER"]) ? (
            <TabPanel value={valueTabs} index={editMode === 'profile' ? 2 : 1}>
              {editMode === "create" ? <p>{intl.formatMessage({ id: "COMPANY.FORM.NO_USER" })}</p> : <OptionsForm editMode={editMode} />}
            </TabPanel>
          ) : (
            <TabPanel value={valueTabs} index={editMode === "profile"
          ...
          {user && (
            <TabPanel value={valueTabs} index={["ROLE_BUYER", "ROLE_VENDOR", "ROLE_TRADER"].includes(user.roles[0]) ? 2 : 3}>
              <BidsForm userId={+id || undefined} classes={classes} isBuyer={user.is_buyer} />
            </TabPanel>
          )}
