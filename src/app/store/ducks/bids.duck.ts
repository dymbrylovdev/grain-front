...
export const actions = {
  ...
  bidsXlsUrlRequest: (id: number) => createAction(BIDS_XLS_URL_REQUEST, { id }),
...
function* bidsXlsUrlSaga({ payload }: { payload: { id: number } }) {
  try {
    const { data }: { data: string } = yield call(() => getBidsXlsUrl(payload.id));
